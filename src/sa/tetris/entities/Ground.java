package sa.tetris.entities;

import sa.tetris.util.Global;
...
public class Ground {
    ...
    public void accept(sa.tetris.entities.Shape shape){
    ...
    public boolean isMovable(sa.tetris.entities.Shape shape, int action){
        int left = shape.getLeft();
        int top = shape.getTop();
        switch (action){
            case sa.tetris.entities.Shape.LEFT:
                left--;
                break;
            case sa.tetris.entities.Shape.RIGHT:
                left++;
                break;
            case sa.tetris.entities.Shape.DOWN:
                top++;
                break;
        }
        for (int x = 0; x < 4; x++){
            for (int y = 0; y < 4; y++){
                if (shape.isMember(x, y, action == sa.tetris.entities.Shape.ROTATE)){
